...
if(isset($_POST['act_save'])){
		...
		$month_num1=$_POST;
		for($j=1;$j<=12;$j++){
			mysql_query("update nelo_plati_avansate set deposit_percent='".$month_num1[$j]."' where month_num=".$j);
		}
		header("location:setari_plata.php");
 }
include("header.php"); 
include("../setari/conf.class.php");
$depo_val='';
	if($neloCore->config['conf_enabled_deposit']==1){
		$depo_val  = 1;
