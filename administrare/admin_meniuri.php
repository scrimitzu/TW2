...
if(isset($_GET['del'])){
	$del2=$_GET['del'];
   $r=mysql_query("select * from nelo_adminmenu where parent_id=".mysql_real_escape_string($_REQUEST['id']));
   if(@mysql_num_rows($r)){
      $msg="<font face=verdana size=1 color=#ffffff><b>You Can't delete.agentmenus exists in this category.</b></font>";	  
   }else{
      $r=mysql_query("delete from nelo_adminmenu where id=\"" .mysql_real_escape_string($_REQUEST['id']). "\"");
      echo"<script>window.location.href='admin_meniuri.php';</script>";
   }
}
?>
...
<tr><td colspan="4" style='background-image:url(imagini/border23.jpg); background-repeat:repeat-x'></td></tr>
<?php
$j=0;
$tids=array();
...
$r=mysql_query("select * from nelo_adminmenu where parent_id=0 order by ord");
	...
	echo"<tr bgcolor=#ffffff class=even>
			<td >";
				if(in_array($d['id'],$tids)):
					...
					$q=mysql_query("select count(*) as total from nelo_adminmenu where  parent_id=" . $d['id']);
    	...
    	if(in_array($d['id'],$tids)):
			$rr=mysql_query("select * from nelo_adminmenu where parent_id=" . $d['id'] . " order by ord");
			$k=0;
