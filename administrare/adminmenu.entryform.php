...
if(isset($_POST['Update'])){
   if(!$_POST['id']):
       $r=mysql_query("insert into nelo_adminmenu(name,url,ord, parent_id) values ('". $_POST['name'] ."','".$_POST['url'] ."',". $_POST['ord'].",". $_POST['parent_id'].")");	
	  // echo "insert into nelo_adminmenu(name,url,ord, parent_id) values ('". $_POST['name'] ."','".$_POST['url'] ."',". $_POST['ord'].",". $_POST['parent_id'].")";
	   $id=mysql_insert_id();
   else:
       $r=mysql_query("update nelo_adminmenu set name='".$_POST['name']."',url='".$_POST['url']."',ord=".$_POST['ord']." where id=". $_POST['id']);
	   $id=$_POST['id'];
	  // echo "update nelo_adminmenu set name='".$_POST['name']."',url='".$_POST['url']."',ord=".$_POST['ord']." where id=". $_POST['id'];
	   
   endif;
   if($r):
   	   $msg="<font face=verdana size=1 color=#111111><b>Update Successful.</b></font>";
       echo"<script>window.location.href='admin_meniuri.php?tid=" . $_REQUEST['tid'] . "';</script>";
   else:
       $msg="<font face=verdana size=1 color=red><b>Update Failure.</b></font>";
   endif;		
}
if(isset($_REQUEST['id'])){
   $r=mysql_query("select * from  nelo_adminmenu where id=" . $_REQUEST['id']);
...
if(isset($_REQUEST['parent_id'])){
   $r=mysql_query("select * from  nelo_adminmenu where id=" . $_REQUEST['parent_id']);
