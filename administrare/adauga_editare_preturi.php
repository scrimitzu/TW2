...
if(isset($_POST['act']) && $_POST['act'] == 1){
	...
	if($_POST['roomtype_edit'] > 0){
		$neloAdminMain->priceplan_edit($_POST['roomtype_edit']);
	}else{
		$neloAdminMain->priceplan_add_edit(); 
	}
	exit;
}
$pageid = 36; 
...
$getHTML2 = array();
$row      = array();
$id=$neloCore->ClearInput($_REQUEST['rtype']);
if($id){
	$text     = '';
	$start_dt = mysql_real_escape_string($_REQUEST['start_dt']);
	if($start_dt != '0000-00-00'){
		
		$row=mysql_fetch_assoc(mysql_query("SELECT bp.*, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date1,
		DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date1, start_date, end_date, bc.title, bc.capacity FROM `nelo_planuri_tarifare` as bp,nelo_capacitate as bc
	...
	$rtypeName = mysql_fetch_assoc(mysql_query("select * from nelo_tip_camera where roomtype_ID='".$row['roomtype_id']."'"));
	
	$getHTML   = $neloAdminMain->getDatepicker($id, $rtypeName['type_name'], $row['start_date'], $row['end_date'], $row);
	...
	$getHTML   = $neloAdminMain->getDatepicker();
...
<span style="font-size:16px; font-weight:bold"><?php echo PRICE_PLAN_ADD_AND_EDIT;?></span>
    <input type="button" value="<?php echo PRICE_PLAN_BACK;?>" onClick="window.location.href='editare_preturi.php'" style="background:#e5f9bb; cursor:pointer; cursor:hand; float:right; "/>
