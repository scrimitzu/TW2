...
$bookingid = $neloCore->ClearInput(base64_decode($_GET['booking_id']));
$viewdetailsquery = mysql_query("select bc.*, bb.* from nelo_rezervari as bb, nelo_clienti as bc where  bb.client_id=bc.client_id and booking_id=".$bookingid."");
...
<div id="container-inside"> <span style="font-size:16px; font-weight:bold"><?php echo VIEW_BOOKING_DETAILS;?> :
  ...
  <?php if(isset($_SERVER['HTTP_REFERER'])){
			...
			if($pathArr['filename'] == 'istoric_arhive_rezervari'){
				echo '<input type="submit" value='.VIEWDETAILS_BACK.' style="background:#e5f9bb; cursor:pointer; cursor:hand; float:right" onClick="javascript:window.location.href=\'istoric_arhive_rezervari.php?book_type='.$_SESSION['book_type'].'\'"/>';
			}else{
  ...
  <?=$neloAdminMain->paymentDetails($rowviewdetails['payment_type'], $bookingid);?><br />
  <table style="font-family:Verdana, Geneva, sans-serif; font-size: 12px; background:#999999; width:700px; border:none;" cellpadding="4" cellspacing="1">
    <tr>
