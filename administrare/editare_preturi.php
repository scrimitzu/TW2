...
if(isset($_REQUEST['pln_del'])){
	...
	$r_id = $neloCore->ClearInput($pln_del[3]);
	mysql_query("delete from nelo_planuri_tarifare where start_date='$pln_del[1]' and end_date='$pln_del[2]' and roomtype_id=".$r_id);
...
<input type="button" value="<?php echo ADD_NEW_PRICEPLAN; ?>" onClick="window.location.href='adauga_editare_preturi.php?rtype=0&start_dt=0'" style="background:#e5f9bb; cursor:pointer; cursor:hand; float:right; " />
        ...
        <tr>
          ...
          <th  colspan="11" align="left"><?php 
							if(isset($_SESSION['roomtype_id'])){
								echo $select_rtype=$neloAdminMain->getRoomtype($_SESSION['roomtype_id']);
							}else{
								echo $select_rtype=$neloAdminMain->getRoomtype();
							}
							?></th>
