...
$get_sub_title=mysql_query("select * from nelo_adminmenu where url='".$filename."'");
if(mysql_num_rows($get_sub_title)){
	$get_sub_title_row=mysql_fetch_array($get_sub_title);
	$get_parent_title=mysql_query("select * from nelo_adminmenu where id='".$get_sub_title_row['parent_id']."'");
...
$main_title="Administrare";
elseif($filename=='schimba_parola.php')
$main_title="Schimba parola";
else
...
<div id="top">
 <div id="top-inside">
  <div id="nelo-addmin-text">Interfata administrare Hotel</div>
  <div class=" top-link_last"><a href="logout.php"><?=HEADER_LOGOUT?></a></div>
  <div class=" top-link"><a href="schimba_parola.php"><?=HEADER_CHANGE_PASS?></a></div>
...
<div id="menu">
 <ul class="menu">
  <?php
  $sql_parent=mysql_query("select * from nelo_adminmenu where parent_id=0 and status='Y' order by ord");
	while($row_parent=mysql_fetch_array($sql_parent))
	{
		...
		$sql_parent222=mysql_query("select * from nelo_adminmenu where parent_id=".$row_parent[0]." and status='Y' order by ord");
