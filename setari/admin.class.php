<?php
$neloAdminMain = new neloAdminCore;
class neloAdminCore{ 
public function global_setting(){
	...
	$select_dt_format="";
	for($p=0; $p<7; $p++){
	if($dt_format_array[$p]==$neloCore->config['conf_dateformat'])
	...
	foreach($room_lock as $key => $value) {
	    if($key==$neloCore->config['conf_booking_exptime'])
	...
	foreach($zonelist as $key => $value) {
	    if($key==$neloCore->config['conf_hotel_timezone'])
	 ...
	 if($neloCore->config['conf_booking_turn_off']==0){
	 ...
	 $select_min_booking="";
	 for($k=1; $k<11; $k++){
	 	if($neloCore->config['conf_min_night_booking']==$k){
	...
	foreach($dformat as $key => $value) {
	    if($value==$neloCore->config['conf_dateformat'])
	...
	private function configure_update($key, $value){
		//echo "update nelo_configurari set conf_value='".$value."' where conf_key='".$key."'";die;
		mysql_query("update nelo_configurari set conf_value='".$value."' where conf_key='".$key."'");
	}
	
	public function payment_gateway(){
		$gateway_value=array();
		$pp_row=mysql_fetch_assoc(mysql_query("select * from nelo_metode_plata where gateway_code='pp'"));
		$poa_row=mysql_fetch_assoc(mysql_query("select * from nelo_metode_plata where gateway_code='poa'"));
		$cc_row=mysql_fetch_assoc(mysql_query("select * from nelo_metode_plata where gateway_code='cc'"));
	...
	public function payment_gateway_post(){
		...
		mysql_query("update nelo_metode_plata set gateway_name='$pp_title', account='$paypal_id', enabled=$pp where gateway_code='pp'");
		mysql_query("update nelo_metode_plata set gateway_name='$poa_title',  enabled=$poa where gateway_code='poa'");
		mysql_query("update nelo_metode_plata set gateway_name='$cc_title',  enabled=$eft where gateway_code='cc'");
	}
	
	public function getEmailContents(){
				global $neloCore;
				$dropList='<option value="0" selected="selected">----'.SELECT_EMAIL_TYPE_CONTENT.'----</option>';
				$sql=mysql_query("select * from nelo_continut_mailuri");
		...
		public function updateEmailContent(){	
		   global $neloCore;
		   $emailsub=$neloCore->ClearInput($_POST['email_sub']);
		   $emailcon=$neloCore->ClearInput($_POST['email_con']);
		   $mailid=$neloCore->ClearInput($_POST['c_update']);
		   mysql_query("update nelo_continut_mailuri set email_subject='".$emailsub."',TEXT_EMAIL='".$emailcon."' where id='".$mailid."'");	
		}
     public function getRoomtype($id=0){
		$roomtype = '<select name="roomtype" id="roomtype"><option value="0">---- '.PRICEPLAN_SELECT.' ----</option>';
		$result = mysql_query("select * from nelo_tip_camera");
 ...
 public function hotel_details_post(){
	...
	public function getBookingInfo($info , $clientid=0, $condition=""){
		...
		switch($info){
			case 1:
			$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, client_id  FROM nelo_rezervari where payment_success=true and CURDATE() <= end_date and is_deleted=false and is_block=false  ".$condition." ";
			break;
		
			case 2:
			$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, client_id, is_deleted  FROM nelo_rezervari where payment_success=true and (CURDATE() > end_date OR is_deleted=true)  and is_block=false ".$condition." ";
			break;
			
			case 3:
			$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, end_date as checkout, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, is_deleted, is_block  FROM nelo_rezervari where client_id=".$clientid;
			break;
	 ...
	 public function getClientInfo($client_id){
		$row=mysql_fetch_assoc(mysql_query("select * from nelo_clienti where client_id=".$client_id));
		return $row;
	 } 
		...
		if($type == 1){
			...
			while($row = mysql_fetch_assoc($result)){
				...
				$html .= '<tr>
							...
							<td width="8%" nowrap>'.$neloCore->config['conf_currency_symbol'].$row['total_cost'].'</td>
							<td width="14%" nowrap>'.$row['booking_time'].'</td>
							<td style="text-align:right; padding:0px 6px 0px 0px" nowrap="nowrap" width="30%">
								<a href="vizualizare_detalii.php?booking_id='.base64_encode($row['booking_id']).'&book_type='.$type.'">'.VIEW_ACTIVE_DETAILS.'</a> | 
			...
			if(mysql_num_rows($result)){
				while($row = mysql_fetch_assoc($result)){
					...
					$html .= '<tr>
								...
								<td width="10%" nowrap>'.$row['start_date'].'</td>
								<td width="10%" nowrap>'.$row['end_date'].'</td>
								<td width="10%" nowrap>'.$neloCore->config['conf_currency_symbol'].$row['total_cost'].'</td>
								<td width="14%" nowrap>'.$row['booking_time'].'</td>
								<td style="text-align:right; padding:0px 6px 0px 0px" nowrap="nowrap" width="30%">
									<a href="vizualizare_detalii.php?booking_id='.base64_encode($row['booking_id']).'&book_type='.$type.'">View Details</a> | 
	...
	public function getRoomtypeCal($id=0){
		$roomtype = '<select name="roomtype" id="roomtype"><option value="0">Toate tipurile</option>';
		$result = mysql_query("select * from nelo_tip_camera");
		while($roomtypeRow=mysql_fetch_assoc($result)){
			if($roomtypeRow['roomtype_ID'] == $id)
				$roomtype .='<option value="'.$roomtypeRow['roomtype_ID'].'" selected="selected">'.$roomtypeRow['type_name'].'</option>';
	...
	public function getCustomerHtml(){
		$html = '';
		$result = mysql_query("select * from nelo_clienti");
		while($row = mysql_fetch_assoc($result)){
			$html .= '<tr><td width="20%" nowrap="nowrap">'.$row['title']." ".$row['first_name']." ".$row['surname'].'</td><td width="30%">'.$row['street_addr'].",".$row['city'].",".$row['country']." - ".$row['zip'].'</td><td width="10%">'.$row['phone'].'</td><td width="25%">'.$row['email'].'</td><td width="15%" align="right" nowrap="nowrap"><a href="rezervariclient.php?client='.base64_encode($row['client_id']).'">'.CUSTOMERLOOKUP_VIEW_BOOKING.'</a>&nbsp;&nbsp;<a href="customerlookupEdit.php?update='.base64_encode($row['client_id']).'">'.CUSTOMERLOOKUP_EDIT.'</a></td></tr>';
		}
		return $html;
	}
...
public function fetchClientBookingDetails($clientid){
		...
		$client_info = mysql_fetch_assoc(mysql_query("select * from nelo_clienti where client_id=".$clientid));
      	while($row =  mysql_fetch_assoc($res)){
			if($row['checkout'] >= date('Y-m-d') && $row['is_deleted'] == 0 && $row['is_block'] == 0){
			  ...
			  $html .= '<tr class="gradeX">
				...
				<td align="right">'.$neloCore->config['conf_currency_symbol'].$row['total_cost'].'</td>
				<td align="right">'.$row['booking_time'].'</td>
				<td align="right">'.$status.'</td>
				<td align="right" nowrap="nowrap"><a href="vizualizare_detalii.php?booking_id='.base64_encode($row['booking_id']).'&book_type='.$type.'"" class="bodytext">'.CUSTOMER_BOOKING_VIEW_DETAILS.'</a>&nbsp;&nbsp;<a  href="javascript:;" onclick="javascript:myPopup2('.$row['booking_id'].');" class="bodytext">'.CUSTOMER_BOOKING_PRINT_VOUCHER.'</a>&nbsp;&nbsp;'.$action.'</td>
			  </tr>';
       }
	   $html .= '</tbody>';	
		//}
	...
	public function booking_cencel_delete($type){
		...
		switch($type){
			case 1:
				...
				$is_cancel = mysql_query("update nelo_rezervari set is_deleted=true where booking_id=".$neloCore->ClearInput($_GET['cancel']));
				if($is_cancel){
				$cust_details = mysql_fetch_assoc(mysql_query("select * from nelo_facturare where booking_id=".$neloCore->ClearInput($_GET['cancel'])));
				$email_details    = mysql_fetch_assoc(mysql_query("select * from nelo_continut_mailuri where id=2"));
				$cancel_emailBody = "Dear ".$cust_details['client_name']."<br>";
				$cancel_emailBody .= html_entity_decode($email_details['TEXT_EMAIL'])."<br>";
				$cancel_emailBody .= "<b>Your Booking Details:</b><br>".$cust_details['invoice']."<br>";
				$cancel_emailBody .= "<b>Regards</b><br>".$neloCore->config['conf_hotel_name']."<BR>".$neloCore->config['conf_hotel_phone']."<br>";
				$neloMail->sendEMail($cust_details['client_email'], $email_details['email_subject'], $cancel_emailBody);
				}
			break;
			
			case 2:
				mysql_query("delete from  nelo_rezervari where booking_id=".$neloCore->ClearInput($_REQUEST['delete']));
				mysql_query("delete from  nelo_rezervari_site where bookings_id=".$neloCore->ClearInput($_REQUEST['delete']));
				mysql_query("delete from  nelo_facturare where booking_id=".$neloCore->ClearInput($_REQUEST['delete']));
			break;
	...
	public function updateCustomerLookup(){
		...
		mysql_query("update nelo_clienti set first_name='".$fname."',surname='".$sname."',title='".$title."',street_addr='".$sadd."',city='".$city."',province='".$province."',zip='".$zip."',country='".$country."',phone='".$phone."',fax='".$fax."',email='".$email."' where client_id=".$cid);	
	...
	public function getCustomerLookup($cid){
		global $neloCore;
		$result = mysql_query("select * from nelo_clienti where client_id=".$neloCore->ClearInput($cid));
	...
	public function paymentDetails($gateway, $bookingid){
		...
		$invoice=mysql_fetch_assoc(mysql_query("select * from nelo_facturare where booking_id=".$bookingid)); 
						...
						$htmlPD .= '<br><table style="font-family:Verdana, Geneva, sans-serif; font-size: 12px; background:#999999; 
									width:700px; border:none;" cellpadding="4" cellspacing="1">
										...
										<tr>
										  ...
										  <td align="left" style="background:#ffffff;">'.$neloCore->decryptCard($ccArr['card_number']).'</td>
										</tr>
										<tr>
	...
	public function getPayment_Gateway($pg){
		$row = mysql_fetch_assoc(mysql_query("select gateway_name from nelo_metode_plata where gateway_code='".$pg."'"));	
		return $row['gateway_name'];
	}
	
	public function getPaypalDetails($bid){
		$row = mysql_fetch_assoc(mysql_query("select payment_txnid, paypal_email from nelo_rezervari where booking_id=$bid"));
		return $row;
	}
	...
	public function getCapacitysql($id=0){
		...
		$sql = "select * from nelo_capacitate".$subquery;
		return $sql;	
	}
	
	public function add_edit_language(){
		global $neloCore;
		$id = $neloCore->ClearInput($_POST['addedit']);
		$lang_title = $neloCore->ClearInput($_POST['lang_title']);
		$lang_code = $neloCore->ClearInput($_POST['lang_code']);
		$lang_file = $neloCore->ClearInput($_POST['lang_file']);
		$lang_default = (isset($_POST['lang_default']))? 1:0;
		if($lang_default==1)
		mysql_query("update nelo_language set lang_default=0");
		
		if($id){
			mysql_query("update nelo_language set lang_title='$lang_title', lang_code='$lang_code', lang_file='$lang_file', lang_default=$lang_default where id=".$id);
		}else{
			mysql_query("insert into `nelo_language` (`lang_title`, `lang_code`, `lang_file`, `lang_default`) values ('$lang_title', '$lang_code', '$lang_file', '$lang_default')");
	...
	public function add_edit_capacity(){
		global $neloCore;
		$id = $neloCore->ClearInput($_POST['addedit']);
		$title = $neloCore->ClearInput($_POST['capacity_title']);
		$capacity = $neloCore->ClearInput($_POST['no_adult']);
		if($id){
			mysql_query("update `nelo_capacitate` set `title`='$title' where `id`=".$id);
		}else{
			mysql_query("insert into `nelo_capacitate` (`title`, `capacity`) values ('$title', ".$capacity.")");
			$capacity_id = mysql_insert_id();
			$result = mysql_query("select start_date, end_date, roomtype_id, default_plan from nelo_planuri_tarifare group by start_date, end_date, roomtype_id");
			if(mysql_num_rows($result)){
				while($row = mysql_fetch_assoc($result)){
					mysql_query("INSERT INTO `nelo_planuri_tarifare` (`roomtype_id`, `capacity_id`, `start_date`, `end_date`, `sun`, `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `default_plan`) VALUES (".$row['roomtype_id'].", ".$capacity_id.", '".$row['start_date']."', '".$row['end_date']."', '0', '0', '0', '0', '0', '0', '0', '".$row['default_plan']."')");
				}
			}else{
				$res = mysql_query($this->getRoomtypesql());
				if(mysql_num_rows($res)){
					while($row = mysql_fetch_assoc($res)){
						mysql_query("INSERT INTO `nelo_planuri_tarifare` (`roomtype_id`, `capacity_id`, `start_date`, `end_date`, `sun`, `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `default_plan`) VALUES (".$row['roomtype_ID'].", '".$capacity_id."', '0000-00-00', '0000-00-00', '0', '0', '0', '0', '0', '0', '0', '1')");
					}
				}
			}
		}
	...
	public function generateCapacityListHtml(){
		...
		while($row = mysql_fetch_assoc($result)){
			$clhtml .= '<tr>
						  <td >'.$row['title'].'</td>
						  <td >'.$row['capacity'].'</td>
						  <td class="center"  align="right"><a href="adauga_editare_capacitate.php?id='.$row['id'].'">'.CAPACITY_LIST_EDIT.'</a> | <a href="javascript:;" onclick="return capacitydelete(\''.$row['id'].'\');">'.CAPACITY_LIST_DELETE.'</a></td>
						</tr>';
		}
		$clhtml .= '</tbody>';
		return $clhtml;
	...
	public function generateLanguageListHtml(){
		$clhtml	= '<tbody>';
		$result = mysql_query("select * from nelo_language");
		while($row = mysql_fetch_assoc($result)){
			...
			$clhtml .= '<tr>
						  ...
						  <td >'.$row['lang_file'].'</td>
						  <td >'.$dflt.'</td>
						  <td class="center"  align="right"><a href="add_edit_setari_traduceri.php?id='.$row['id'].'">'.LANGAUGE_LIST_EDIT.'</a> | <a href="manager_limbi.php?delid='.$row['id'].'" >'.LANGAUGE_LIST_DELETE.'</a></td>
						</tr>';
		}
		$clhtml .= '</tbody>';
		return $clhtml;
	...
	public function delete_capacity(){
		global $neloCore;
		mysql_query("delete from nelo_capacitate where id=".$neloCore->ClearInput($_GET['cdelid']));
		mysql_query("delete from nelo_camere where capacity_id=".$neloCore->ClearInput($_GET['cdelid']));
		mysql_query("delete from nelo_planuri_tarifare where capacity_id=".$neloCore->ClearInput($_GET['cdelid']));	
	}
	
	public function delete_lang(){
		global $neloCore;
		mysql_query("delete from nelo_language where id=".$neloCore->ClearInput($_GET['delid']));	
	}
	...
	public function getRoomtypesql($id=0){
		...
		$sql = "select * from nelo_tip_camera".$subquery;
		return $sql;	
	}
			...
			if(($_FILES['img']['name'] != "")){
				...
				foreach($_FILES as $k => $v){ 	
					...
					if( $_FILES[$k]['error'] == 0 && $_FILES[$k]['size'] < $max_image_size ){
						...
						mysql_query("update `nelo_tip_camera` set `img`='$img_rname' where `roomtype_ID`=".$id);
						if($enable_thumbnails) $this->make_thumbnails($upload_dir, $img_rname, $newWidth, 50);						
					}
				}
			}			
			mysql_query("update `nelo_tip_camera` set `type_name`='$title' where `roomtype_ID`=".$id);
		}else{
			mysql_query("insert into `nelo_tip_camera` (`type_name`) values ('$title')");
			$roomtype_id = mysql_insert_id();
			$result = mysql_query("select start_date, end_date, capacity_id, default_plan from nelo_planuri_tarifare group by start_date, end_date, capacity_id");
			if(mysql_num_rows($result)){
				while($row = mysql_fetch_assoc($result)){
					mysql_query("INSERT INTO `nelo_planuri_tarifare` (`roomtype_id`, `capacity_id`, `start_date`, `end_date`, `sun`, `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `default_plan`) VALUES (".$roomtype_id.", '".$row['capacity_id']."', '".$row['start_date']."', '".$row['end_date']."', '0', '0', '0', '0', '0', '0', '0', '".$row['default_plan']."')");
				}
			}else{
				$res = mysql_query($this->getCapacitysql());
				if(mysql_num_rows($res)){
					while($row = mysql_fetch_assoc($res)){
						mysql_query("INSERT INTO `nelo_planuri_tarifare` (`roomtype_id`, `capacity_id`, `start_date`, `end_date`, `sun`, `mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `default_plan`) VALUES (".$roomtype_id.", '".$row['id']."', '0000-00-00', '0000-00-00', '0', '0', '0', '0', '0', '0', '0', '1')");
					}
				}
			}
		}
	...
	public function generateRoomtypeListHtml(){
		...
		while($row = mysql_fetch_assoc($result)){
			$rthtml .= '<tr>
						  <td>'.$row['type_name'].'</td>
						  <td class="center"  align="right"><a href="adauga_editare_tip_camere.php?id='.$row['roomtype_ID'].'">'.ROOM_TYPE_EDIT.'</a> | <a href="tip_camere.php" onclick="return deleteRoomType(\''.$row['roomtype_ID'].'\');">'.ROOM_TYPE_DELETE.'</a></td>
						</tr>';
		}
		$rthtml .= '</tbody>';
		return $rthtml;
	...
	public function delete_roomtype(){
		global $neloCore;
		mysql_query("delete from nelo_tip_camera where roomtype_ID=".$neloCore->ClearInput($_GET['rdelid']));
		mysql_query("delete from nelo_camere where roomtype_id=".$neloCore->ClearInput($_GET['rdelid']));
		mysql_query("delete from nelo_planuri_tarifare where roomtype_id=".$neloCore->ClearInput($_GET['rdelid']));	
	}
	...
	public function getRoomsql($rid=0, $cid=0){
		...
		$sql = "SELECT count(*) as NoOfRoom, `roomtype_id`, `capacity_id` FROM nelo_camere".$subquery." group by `roomtype_id`, `capacity_id`";
		return $sql;	
	}
	...
	public function generateRoomListHtml(){
		...
		while($row = mysql_fetch_assoc($result)){
			...
			$rthtml .= '<tr>
						  ...
						  <td>'.$row['NoOfRoom'].'</td>
						  <td class="center" align="right"><a href="adauga_editare_camera.php?rid='.$row['roomtype_id'].'&cid='.$row['capacity_id'].'">'.EDIT_ROOM_LIST.'</a> | <a href="'.$_SERVER['PHP_SELF'].'?rid='.$row['roomtype_id'].'&cid='.$row['capacity_id'].'">'.DELETE_ROOM_LIST.'</a></td>
						</tr>';
		}
		$rthtml .= '</tbody>';
		return $rthtml;
	...
	public function add_edit_room(){
		global $neloCore;
		$no_of_room   = $neloCore->ClearInput($_POST['no_of_room']);
		$pre_room_cnt = $neloCore->ClearInput($_POST['pre_room_cnt']);
		$roomtypeId   = $neloCore->ClearInput($_POST['roomtype_id']);
		$capacityId   = $neloCore->ClearInput($_POST['capacity_id']);
		
		if($pre_room_cnt != "" || $pre_room_cnt != NULL){
			if($no_of_room > $pre_room_cnt){
				$limit = $no_of_room - $pre_room_cnt;
				for($i=1; $i<=$limit; $i++){
					mysql_query("insert into nelo_camere (`roomtype_id`, `room_no`, `capacity_id`) values (".$roomtypeId.", '1', ".$capacityId.")");
					$room_id = mysql_insert_id();
					mysql_query("update nelo_camere set room_no='".$room_id."' where room_ID=".$room_id);				
				}
			}
			
			if($no_of_room < $pre_room_cnt){
				$limit = $pre_room_cnt - $no_of_room;
				mysql_query("delete from nelo_camere where roomtype_id=".$roomtypeId." and capacity_id=".$capacityId." limit ".$limit);
			}	
		}else{
			$result  = mysql_query("select * from nelo_camere where roomtype_id=".$roomtypeId." and capacity_id=".$capacityId);
			if(!mysql_num_rows($result)){		
	...
	public function delete_room(){
		global $neloCore;
		mysql_query("delete from nelo_camere where roomtype_id=".$neloCore->ClearInput($_GET['rid'])." and capacity_id=".$neloCore->ClearInput($_GET['cid']));
	}
	
	public function getBlockRoomDetails(){
		global $neloCore;
		$getHtml='<tbody>';
		$result=mysql_query("SELECT br.bookings_id, bb.block_name, DATE_FORMAT(bb.start_date, '".$neloCore->userDateFormat."') AS StartDate, DATE_FORMAT(bb.end_date, '".$neloCore->userDateFormat."') AS EndDate, br.room_type_id, brt.type_name, br.room_id, brm.capacity_id, count(*) as NoOfRoom from nelo_rezervari_site br, nelo_rezervari bb, nelo_tip_camera brt, nelo_camere brm where br.bookings_id=bb.booking_id and bb.is_block=1 and br.room_type_id=brt.roomtype_id and br.room_id=brm.room_ID group by br.room_type_id, brm.capacity_id");
		if(mysql_num_rows($result)){
			while($row=mysql_fetch_assoc($result)){
				$row_capacity_title=mysql_fetch_assoc(mysql_query("SELECT bc.title from nelo_capacitate bc, nelo_camere br where br.capacity_id=bc.id and br.room_id='".$row['room_id']."'"));
	...
	public function getDatepicker($id=0, $roomtypename='', $startdate='', $enddate='', $row=array()){
		...
		if($id){
			$html = '<tr>
			  <td width="80px"><strong>&nbsp;Tip camera:</strong></td>
			...
			$editPriceplanHTML = '<table cellpadding="3" cellspacing="0" border="0" style="font-family:Arial, Helvetica, sans-serif; font-size:12px;" width="700px;">
				<tr>
					<td width="85px"><strong>Valabil</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Luni</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Marti</strong></td>
					<td width="75px" style="padding-left:10px;"><strong>Miercuri</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Joi</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Vineri</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Sambata</strong></td>
					<td width="80px" style="padding-left:10px;"><strong>Duminica</strong></td>
				</tr>
				<tr><td colspan="8"><hr/></td></tr><tr>
<td>'.$row['title'].' ('.$row['capacity'].') &nbsp;</td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="sun" id="sun" class="required number" value="'.$row['sun'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="mon" id="mon" class="required number" value="'.$row['mon'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="tue" id="tue" class="required number" value="'.$row['tue'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="wed" id="wed" class="required number" value="'.$row['wed'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="thu" id="thu" class="required number" value="'.$row['thu'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="fri" id="fri" class="required number" value="'.$row['fri'].'" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" name="sat" id="sat" class="required number" value="'.$row['sat'].'" size="4" /></td>
				</tr></table><br>
				<table width="100%">
					<tr><td width="82px"></td><td style="padding-left:10px;"><input type="submit" value="Actualizare" style="background: #EFEFEF;"/></td></tr>
					<tr><td width="82px"></td><td style="padding-left:10px;">&nbsp;</td></tr>
					<tr><td colspan="2"><font color="#FF0000"><b>*</b></font>  &nbsp;&nbsp;&nbsp;Camp obligatoriu</td></tr>
					<tr><td colspan="2"><font color="#FF0000"><b>**</b></font> &nbsp;Doar cifre</td></tr>
				</table>';
			...
			$html = '<tr>
			  <td width="95px"><strong>Tipul Camerei:</strong></td>
			  <td>&nbsp;'.$this->generateRoomtypecombo().'</td>
			</tr>
			<tr id="daybyplan1">
			  <td><strong>Start date:</strong></td>             
			  <td valign="middle"><table><tr><td><input type="text" id="txtFromDate" name="startdate" readonly="readonly" size="10"/></td><td style="padding-left:5px;"><a id="datepickerImage" href="javascript:;"><img src="../imagini/month.png" height="18px" width="18px" /></a></td></tr></table></td>
			</tr>
			<tr id="daybyplan2">
			  <td><strong>End date:</strong></td>
			  <td><table><tr><td><input type="text" id="txtToDate" name="closingdate" value="" readonly="readonly"  size="10"></td><td style="padding-left:5px;"><a id="datepickerImage1" href="javascript:;"><img src="../imagini/month.png" height="18px" width="18px" /></a></td></tr></table></td>
			</tr>';
	...
	public function priceplan_add_edit(){
		global $neloCore;
		$roomtype       = $neloCore->ClearInput($_POST['roomtype_id']);
		$start_date_old = $_POST['start_date_old'];
		$startdate      = $neloCore->getMySqlDate(mysql_real_escape_string($_POST['startdate']));
		$closingdate    = $neloCore->getMySqlDate(mysql_real_escape_string($_POST['closingdate']));
		if($startdate == "" && $closingdate == ""){
			...
			header("location:editare_preturi.php");
		}else{
			$startdate     = $startdate;
			$closingdate   = $closingdate;
			$default_plan  = 0;
		}
		$exist = mysql_num_rows(mysql_query("select * from nelo_planuri_tarifare where roomtype_id=$roomtype and (('$startdate'  Between
	...
	public function priceplan_edit($plan_id){
		$row = mysql_fetch_assoc(mysql_query("select roomtype_id from nelo_planuri_tarifare where plan_id=".$plan_id));
		$_SESSION['roomtype_id'] = $row['roomtype_id'];
		mysql_query("UPDATE `nelo_planuri_tarifare` SET 
					`sun` = '".$_POST['sun']."',
					`mon` = '".$_POST['mon']."',
					`tue` = '".$_POST['tue']."',
					`wed` = '".$_POST['wed']."',
	...
	public function getCreditCardDetails($bid){
		$result = mysql_query("select * from nelo_informatii_cc where booking_id='".$bid."'");
		if(mysql_num_rows($result)){
	...
	public function changePassword(){
		...
		$adminid = $_SESSION['cpuidnelo'];
		$result  = mysql_query("select * from nelo_admin where pass=\"" . md5($oldpass) . "\" and id=".$adminid);
		if(@mysql_num_rows($result)){
			mysql_query("update nelo_admin set pass='".md5($newpass)."' where id=".$adminid);
	...
	public function homewidget($type){
			global $neloCore;
			if($type==1){
				$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, client_id  FROM nelo_rezervari where payment_success=true and is_block=false  order by booking_id desc limit 10";
				//echo $sql;
			}else if($type==2){
				$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, client_id  FROM nelo_rezervari where payment_success=true and is_block=false and DATE_FORMAT(start_date, '%Y-%m-%d')=CURDATE()";
			}else if($type==3){
				$sql = "SELECT booking_id, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$neloCore->userDateFormat."') AS booking_time, payment_type, client_id  FROM nelo_rezervari where payment_success=true and is_block=false  and DATE_FORMAT(end_date, '%Y-%m-%d')=CURDATE()";
			}
			...
			while($row = mysql_fetch_assoc($result)){
				...
				$html .= '<tr>
							...
							<td style="text-align:right; padding:0px 6px 0px 0px" nowrap="nowrap" width="15%">
								<a href="vizualizare_detalii.php?booking_id='.base64_encode($row['booking_id']).'&book_type=1">'.VIEW_ACTIVE_DETAILS.'</a>  
