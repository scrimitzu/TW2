...
class adminAjaxProcessor
{
	...
	public function getneloEmailcontent(){
		global $neloCore;
		$errorcode = 0;
		$strmsg = "";
		$getArray = array();
		$choiceid = $neloCore->ClearInput($_POST['choiceid']);
		$result = mysql_query("select * from nelo_continut_mailuri where id='".$choiceid."'");
		if(mysql_num_rows($result)){
			$getEmailcontentlist=mysql_fetch_assoc($result);
			$email = $getEmailcontentlist['email_subject'];
			$emailText = $getEmailcontentlist['TEXT_EMAIL'];
	...
	public function generatePriceplanList(){
		...
		$strmsg	= "";
		$pphtml='';
		$roomtype_id = $neloCore->ClearInput($_POST['roomtype_id']);
		$daterange = mysql_query("select start_date, end_date, DATE_FORMAT(start_date, '".$neloCore->userDateFormat."') AS start_date1, DATE_FORMAT(end_date, '".$neloCore->userDateFormat."') AS end_date1, default_plan from nelo_planuri_tarifare where roomtype_id='".$roomtype_id."' group by start_date, end_date");
		
		  if(mysql_num_rows($daterange)){
			while($row_daterange = mysql_fetch_assoc($daterange)){
				$query = mysql_query("select bp.*, bc.title from nelo_planuri_tarifare as bp, nelo_capacitate as bc where start_date='".$row_daterange['start_date']."' and end_date='".$row_daterange['end_date']."' and roomtype_id='".$roomtype_id."' and bp.capacity_id=bc.id");
					...
					$i1=$daletetd;			
					  while($row_pp=mysql_fetch_assoc($query)){
						   $pphtml.='<tr class=odd  bgcolor="#E8E8E8">
									...
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['sun'].'</font></td>
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['mon'].'</font></td>
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['tue'].'</font></td>
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['wed'].'</font></td>    
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['thu'].'</font></td>  
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['fri'].'</font></td> 
									<td align="left" width="70px"><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1">'.$neloCore->config['conf_currency_symbol'].$row_pp['sat'].'</font></td>
									<td align="left" width="90px" colspan="2" style="padding-left:10px;"><b><font color="#666666"  face="Verdana, Arial, Helvetica, sans-serif" size="1"><a href="adauga_editare_preturi.php?rtype='.$row_pp['plan_id'].'&start_dt='.$row_pp['start_date'].'">'.EDIT_ROOM_LIST.'</a></font></b></td>';
								if($daletetd==$i1){	
	...
	public function getdefaultcapacity(){
		...
		$strmsg = "";
		$roomtype_id = $neloCore->ClearInput($_POST['roomtype_id']);	
		$capacity_sql = mysql_query("select * from nelo_capacitate");
		if(mysql_num_rows($capacity_sql)){
			...
			$priceplan_sql = mysql_query("SELECT * FROM `nelo_planuri_tarifare` WHERE roomtype_id='".$roomtype_id."' and capacity_id='".$row['id']."'");		
			if(mysql_num_rows($priceplan_sql)){
				$priceplan_row = mysql_fetch_assoc($priceplan_sql);
				$capacity_input_box.=
				'<tr>
<td>'.$row['title'].' ('.$row['capacity'].') &nbsp;</td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][sun]" id="priceplan['.$row['id'].'][sun]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][mon]" id="priceplan['.$row['id'].'][mon]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][tue]" id="priceplan['.$row['id'].'][tue]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][wed]" id="priceplan['.$row['id'].'][wed]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][thu]" id="priceplan['.$row['id'].'][thu]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][fri]" id="priceplan['.$row['id'].'][fri]"  size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][sat]" id="priceplan['.$row['id'].'][sat]"  size="4" /></td>
				</tr>';
		}else{
			  $capacity_input_box.=
				'<tr>
...
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][thu]" id="priceplan['.$row['id'].'][thu]" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][fri]" id="priceplan['.$row['id'].'][fri]" size="4" /></td>
<td>'.$neloCore->config['conf_currency_symbol'].'<input type="text" class="required number"  name="priceplan['.$row['id'].'][sat]" id="priceplan['.$row['id'].'][sat]" size="4" /></td></tr>';
			}
		}
	...
	public function getDeposit()
	{
		...
		$getddresult="";
		$type=$_POST['type'];
		$deposit_discount_result=mysql_query("SELECT * FROM nelo_plati_avansate");
...
mysql_query("update nelo_configurari set conf_value='1' where conf_key='conf_enabled_deposit'");					 
				 }else{
					     mysql_query("update nelo_configurari set conf_value='0' where conf_key='conf_enabled_deposit'");
	...
	public function genearateCapacityCombo(){
		...
		$errorcode = 0;
		$strmsg = "";
		$roomtype = $neloCore->ClearInput($_POST['roomtype']);
		$result = mysql_query("select distinct br.capacity_id, bc.title from nelo_camere br, nelo_capacitate bc where roomtype_id=".$roomtype." and bc.id=br.capacity_id");
		if(mysql_num_rows($result)){
			$chtml = '<select name="capacity_id" id="capacity_id"><option value="0">Toate</option>';
