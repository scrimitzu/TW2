...
class neloSearch
{
	...
	private function setRequestParams() {		
		...
		$this->setMyParamValue($this->checkInDate, $neloCore->ClearInput($tmpVar), NULL, true);
		$tmpVar = isset($_POST['check_out'])? $_POST['check_out'] : NULL; 
		$this->setMyParamValue($this->checkOutDate, $neloCore->ClearInput($tmpVar), NULL, true); 
	    $tmpVar = isset($_POST['capacity'])? $_POST['capacity'] : 0;
		$this->setMyParamValue($this->oaspetiPerCamera, $neloCore->ClearInput($tmpVar), 0, true); 
				
		$this->mysqlCheckInDate = $neloCore->getMySqlDate($this->checkInDate);   	  
		$this->mysqlCheckOutDate = $neloCore->getMySqlDate($this->checkOutDate);	  			
	}
	...
	private function setMySessionVars(){
		...
		$_SESSION['sv_numarNopti'] = $this->numarNopti;		
		$_SESSION['sv_guestperroom'] = $this->oaspetiPerCamera;		
	...
	private function getnumarNopti() {		
		...
		$this->numarNopti = round(abs($checkin_date_new - $checkout_date_new) / 86400);
	}
	...
	private function checkSearchEngine(){
		global $neloCore;
		if(intval($neloCore->config['conf_booking_turn_off']) > 0){
		...
		if($dateDiff < 0){
			...
			return 0;
		}else if($dateDiff < intval($neloCore->config['conf_min_night_booking'])){
	...
	private function loadRoomTypes() {			
		$sql = mysql_query("SELECT * FROM nelo_tip_camera");
	...
	private function loadMultiCapacity() {	
		$sql = mysql_query("SELECT * FROM nelo_capacitate WHERE capacity >= ".$this->oaspetiPerCamera);
		while($currentrow = mysql_fetch_assoc($sql)){			
	...
	public function getAvailableRooms($roomTypeId, $roomTypeName, $capcityid){
		global $neloCore;		
		$currency_symbol = $neloCore->config['conf_currency_symbol'];		
		...
		if($tmpctr > 1){
			...
			if($_month == $month_){
			 ...
			 $price_details_html.='<td bgcolor='.$color_.' align="right"><b>'.$this->numarNopti.' Nopti</b></td></tr>';
			 foreach($totalDays[0] as $date2 => $val){					
				$pricesql = mysql_query("SELECT * FROM nelo_planuri_tarifare WHERE roomtype_id = ".$roomTypeId." AND capacity_id = ".$capcityid." AND ('".$val."' BETWEEN start_date AND end_date)");
				if(mysql_num_rows($pricesql)){
					$row=mysql_fetch_assoc($pricesql);
				}else{
					$pricesql2 = mysql_query("SELECT * FROM nelo_planuri_tarifare WHERE roomtype_id = ".$roomTypeId." AND capacity_id = ".$capcityid." AND  default_plan=1");
			...
			if($neloCore->config['conf_tax_amount'] > 0 && $neloCore->config['conf_price_with_tax']==1){ 
				$price_details_html.='<td bgcolor='.$_color.'>'.$mon.'</td>';
				foreach($dayName as $days => $totalnum){
					$pricewithtax=$$days+(($$days * $neloCore->config['conf_tax_amount'])/100);
					if($days == 'Sat' || $days == 'Sun'){
