...
class BookingProcess
{
	...
	private function setMyRequestParams(){ 
		global $neloCore;
		$this->setMyParamValue($this->oaspetiPerCamera, 'SESSION', 'sv_guestperroom', 0, true);	
		...
		$this->setMyParamValue($this->noOfNights, 'SESSION', 'sv_numarNopti', 0, true);
		...
		$this->bookingId		= time();		
		$this->expTime 			= intval($neloCore->config['conf_booking_exptime']);	
		$this->currencySymbol 	= $neloCore->config['conf_currency_symbol'];
		$this->taxPercent 		= $neloCore->config['conf_tax_amount'];
		...
		$this->taxWithPrice     = $neloCore->config['conf_price_with_tax'];
	...
	private function removeSessionVariables(){
		...
		if(isset($_SESSION['sv_numarNopti'])) unset($_SESSION['sv_numarNopti']);
	...
	private function checkAvailability(){
		$sql = "
		SELECT resv.room_id
		  FROM nelo_rezervari_site resv, nelo_rezervari boks
	...
	private function saveBookingData(){
		$sql = mysql_query("INSERT INTO nelo_rezervari (booking_id, booking_time, start_date, end_date, client_id, total_cost, payment_amount, payment_type, special_requests) values(".$this->bookingId.", NOW(), '".$this->mysqlCheckInDate."', '".$this->mysqlCheckOutDate."', ".$this->clientId.", ".$this->grandTotalAmount.", ".$this->totalPaymentAmount.", '".$this->paymentGatewayCode."', '".$this->clientdata['message']."')");
		
		foreach($this->reservationdata as $revdata){
			foreach($revdata['availablerooms'] as $rooms){				
			$sql = mysql_query("INSERT INTO nelo_rezervari_site (bookings_id, room_id, room_type_id) values(".$this->bookingId.",  ".$rooms['roomid'].", ".$revdata['roomtypeid'].")");
			} 
		}
	}	
	
	private function createInvoice(){
		$this->invoiceHtml = '<table style="font-family:Verdana, Geneva, sans-serif; font-size: 12px; background:#999999; width:700px; border:none;" cellpadding="4" cellspacing="1"><tbody><tr><td align="left" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;" colspan="4">'.mysql_real_escape_string(TEXT_REZERVARI_DETALII).'</td></tr>
		<tr><td align="left" style="background:#ffffff;">'.mysql_real_escape_string(TEXT_NUMAR_REZERVARE).'</td><td align="left" style="background:#ffffff;" colspan="3">'.$this->bookingId.'</td></tr>
		<tr><td align="left" style="background:#ffffff;">'.mysql_real_escape_string(TEXT_NUMELE_CLIENTULUI).'</td><td align="left" style="background:#ffffff;" colspan="3">'.$this->clientName.'</td></tr>	
		<tr height="8px;"><td align="left" style="background:#ffffff;" colspan="4"></td></tr>
		<tr><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_DATA_CHECK_IN).'</td><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_DATA_CHECK_OUT).'</td><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_NUMAR_NOPTI).'</td><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_NUMAR_CAMERE).'</td></tr>
		...
		<tr height="8px;"><td align="left" style="background:#ffffff;" colspan="4">&nbsp;</td></tr>
		<tr><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(NUMBER_OF_ROOM_TEXT).'</td><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_TIPUL_CAMEREI).'</td><td align="center" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_NUMAR_MAXIM_OCUPANTI).'</td><td align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_TOTAL_BRUT).'</td></tr>';		
			
		foreach($this->reservationdata2 as $revdata){
				$this->invoiceHtml.= '<tr><td align="center" style="background:#ffffff;">'.$revdata['roomno'].'</td><td align="center" style="background:#ffffff;">'.$revdata['roomtype'].' ('.$revdata['capacitytitle'].')</td><td align="center" style="background:#ffffff;">'.$revdata['capacity'].' '.TEXT_ADULTI_A.'';			
				$this->invoiceHtml.= '</td><td align="right" style="background:#ffffff;">'.$this->currencySymbol.number_format($revdata['grosstotal'], 2 , '.', ',').'</td></tr>';
			
		}
		
		$this->invoiceHtml.= '<tr height="8px;"><td align="left" style="background:#ffffff;" colspan="4"></td></tr><tr><td colspan="3" align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_SUBTOTAL).'</td><td align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.$this->currencySymbol.number_format($this->pricedata['subtotal'], 2 , '.', ',').'</td></tr>';
					
		if($this->taxPercent > 0 &&  $this->taxWithPrice == 0){ 	
		$this->invoiceHtml.= '<tr><td colspan="3" align="right" style="background:#ffffff;">'.mysql_real_escape_string(TEXT_TAXE).'('.number_format($this->taxPercent, 2 , '.', '').'%)</td><td align="right" style="background:#ffffff;">(+) '.$this->currencySymbol.number_format($this->taxAmount, 2 , '.', ',').'</td></tr><tr><td colspan="3" align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_TOTAL_GENERAL).'</td><td align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.$this->currencySymbol.number_format($this->grandTotalAmount, 2 , '.', ',').'</td></tr>';
		}else{
			$this->invoiceHtml.= '<tr><td colspan="3" align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.TEXT_TOTAL_GENERAL.'</td><td align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.$this->currencySymbol.number_format($this->grandTotalAmount, 2 , '.', ',').'</td></tr>';	
		}
		if($this->depositenabled && ($this->pricedata['advancepercentage'] > 0 && $this->pricedata['advancepercentage'] < 100)){
			
			$this->invoiceHtml.= '<tr><td colspan="3" align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.mysql_real_escape_string(TEXT_PLATI_AVANSATE).'(<span style="font-size: 10px;">'.number_format($this->pricedata['advancepercentage'], 2 , '.', '').'% '.mysql_real_escape_string(TEXT_DIN_TOTAL_PLATA).'</span>)</td><td align="right" style="font-weight:bold; font-variant:small-caps; background:#eeeeee;">'.$this->currencySymbol.number_format($this->totalPaymentAmount, 2 , '.', ',').'</td></tr>';
		} 
		$this->invoiceHtml.= '</tbody></table>';
		
		if($this->paymentGatewayCode == "poa"){
			$payoptions = "Manual: Plata la livrare";		
		...
		/* insert the invoice data in nelo_facturare table */
		$insertInvoiceSQL = mysql_query("INSERT INTO nelo_facturare(booking_id, client_name, client_email, invoice) values(".$this->bookingId.", '".$this->clientName."', '".$this->clientdata['email']."', '".$this->invoiceHtml."')");	
	}
}
?>