...
*/
$neloCore = new neloHotelCore;
class neloHotelCore{
	public $config = array();
	public $userDateFormat = "";		
	
	function neloHotelCore(){		
		$this->getneloConfig();
		$this->getUserDateFormat();		
	}	
	
	private function getneloConfig(){
		$sql = mysql_query("SELECT conf_id, IFNULL(conf_key, false) AS conf_key, IFNULL(conf_value,false) AS conf_value FROM nelo_configurari");
	...
	public function capacitycombo(){
		...
		$capacityrow = mysql_fetch_assoc(mysql_query("SELECT Max(capacity) as capa FROM nelo_capacitate WHERE `id` IN (SELECT DISTINCT (capacity_id) FROM nelo_camere) ORDER BY capacity"));
	...
	public function clearExpiredBookings(){		
		$sql = mysql_query("SELECT booking_id FROM nelo_rezervari WHERE payment_success = false AND ((NOW() - booking_time) > ".intval($this->config['conf_booking_exptime'])." )");
		while($currentRow = mysql_fetch_assoc($sql)){			
			mysql_query("DELETE FROM nelo_facturare WHERE booking_id = '".$currentRow["booking_id"]."'");
			mysql_query("DELETE FROM nelo_rezervari_site WHERE bookings_id = '".$currentRow["booking_id"]."'");	
			mysql_query("DELETE FROM nelo_rezervari WHERE booking_id = '".$currentRow["booking_id"]."'");			
		}
		mysql_free_result($sql);
	}
	...
	public function loadPaymentGateways() {			
		...
		$sql = mysql_query("SELECT * FROM nelo_metode_plata where enabled=true");
		while($currentRow = mysql_fetch_assoc($sql)){	
	...
	public function paymentGateway($code){
		$row = mysql_fetch_assoc(mysql_query("SELECT gateway_name FROM nelo_metode_plata where gateway_code='".$code."'"));
		return  $row['gateway_name'];
	}
	
	public function getInvoiceinfo($bid){
		$invoiceres=mysql_fetch_assoc(mysql_query("select * from nelo_facturare where booking_id='".$bid."'"));			
		return $invoiceres['invoice'];
	}
	public function paymentGatewayName($gcode){
		$row=mysql_fetch_row(mysql_query("select gateway_name from nelo_metode_plata where gateway_code='".$gcode."'"));
		return $row[0];
	}
}